...
if args.lagrangian:
    ...
    name_append = ''
log_path = make_model_path('%d_%.2f_%.2f_%.2f%s' % (args.train_size, args.mi, args.epsilon1, args.epsilon2, name_append))
...
lambda1 = tf.get_variable('lambda1', shape=[], dtype=tf.float32, initializer=tf.constant_initializer(2.0))
lambda2 = tf.get_variable('lambda2', shape=[], dtype=tf.float32, initializer=tf.constant_initializer(2.0))
# Cannot simply use clipping here because once at max/min there is no more gradient
lambda1_ub = tf.assign(lambda1, 100.0)
lambda1_lb = tf.assign(lambda1, 0.0)
lambda2_ub = tf.assign(lambda2, 100.0)
lambda2_lb = tf.assign(lambda2, 0.0)

loss_all = lambda1 * loss_nll + (lambda2 + args.mi - lambda1) * loss_mmd + (lambda1 - args.mi) * loss_elbo - args.epsilon1 * lambda1 - args.epsilon2 * lambda2
