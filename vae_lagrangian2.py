...
lambda1_ub = tf.assign(lambda1, tf.minimum(lambda1, 200.0))
lambda1_lb = tf.assign(lambda1, tf.maximum(lambda1, 0.0))
lambda2_ub = tf.assign(lambda2, tf.minimum(lambda2, 200.0))
lambda2_lb = tf.assign(lambda2, tf.maximum(lambda2, 0.0))
lambda_clip = [lambda1_ub, lambda1_lb, lambda2_ub, lambda2_lb]
...
lambda_update = tf.train.GradientDescentOptimizer(1e-3).minimize(-loss_all, var_list=lambda_vars)
